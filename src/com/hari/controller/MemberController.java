...
public class MemberController implements MemberControllerImpl {
	...
	@Override
	@RequestMapping(value = "/addMemHari", method = RequestMethod.POST)
	public Member registerHari(@RequestParam("name") String name, @RequestParam("password") String password, @RequestParam("type") String type, @RequestParam("add") String add, @RequestParam("mail") String mail, @RequestParam("cno") int cno) {
		try(ClassPathXmlApplicationContext ctx = new ClassPathXmlApplicationContext("applicationContext.xml")) {
			int id = 1;
			String status2 = "Active";
			dao=(MemberDao)ctx.getBean("mdao");
			...
			if(status == -99) {
				...
				System.out.println("Member added");
				//url = "http://localhost:8080/PAF/AddMemSucc.jsp?memN="+name;
				return mem;
			}
		}
		catch (Exception e) {
	...
	@Override
	public String registerUser(String name, String password, String type, String add, String mail, int cno) {
		// TODO Auto-generated method stub
	...
	@Override
	public String updateUser(String name,String type,String add,String mail,int cno){
